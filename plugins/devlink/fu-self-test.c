...
fu_devlink_device_func(void)
{
	...
	if (!ret) {
		g_test_message("Device probe failed (expected if devlink not available): %s", error->message);
...
fu_devlink_device_flash_func(void)
{
	...
	g_test_message("Testing firmware flash for netdevsim/netdevsim1");
	...
	if (!ret) {
		g_test_message("Firmware flash failed (expected if netdevsim not available): %s", error->message);
		/* Don't fail the test if netdevsim isn't available */
		return;
	}

	g_test_message("Firmware flash completed successfully!");
