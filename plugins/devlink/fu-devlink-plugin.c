...
fu_devlink_plugin_device_added_from_netlink(FuDevlinkPlugin *self, const struct nlmsghdr *nlh)
{
	...
	mnl_attr_parse(nlh, sizeof(struct genlmsghdr), fu_devlink_attr_cb, tb);
	...
	device = fu_devlink_device_new(fu_plugin_get_context(FU_PLUGIN(self)), bus_name, dev_name);

	/* Use FuDeviceLocker to open device (which triggers setup and ensures device ID) */
	{
		g_autoptr(FuDeviceLocker) locker = fu_device_locker_new(FU_DEVICE(device), &error);
		if (locker == NULL) {
			g_debug("failed to open devlink device %s/%s: %s",
				bus_name,
				dev_name,
				error->message);
			return;
		...
		/* Add device to plugin - device ID is now available */
		fu_plugin_device_add(FU_PLUGIN(self), FU_DEVICE(device));
		fu_plugin_cache_add(FU_PLUGIN(self),
...
fu_devlink_plugin_netlink_cb(GIOChannel *channel, GIOCondition condition, gpointer user_data)
{
	...
	if (rc == -1)
		g_debug("failed to process netlink message: %s", g_strerror(errno));
...
fu_devlink_plugin_setup_netlink(FuDevlinkPlugin *self, GError **error)
{
	...
	if (!fu_devlink_gen_socket_open(&self->nlg, error))
		return FALSE;

	/* Subscribe to devlink multicast notifications */
	if (!fu_devlink_mcast_group_subscribe(&self->nlg)) {
		g_set_error(error,
			    FWUPD_ERROR,
			    FWUPD_ERROR_NOT_SUPPORTED,
			    "failed to subscribe to devlink notifications");
		fu_devlink_gen_socket_close(&self->nlg);
		return FALSE;
	}
...
static int
fu_devlink_enumerate_cb(const struct nlmsghdr *nlh, void *data)
{
	FuDevlinkPlugin *self = FU_DEVLINK_PLUGIN(data);
	struct genlmsghdr *genl;
...
fu_devlink_plugin_enumerate_devices(FuDevlinkPlugin *self, GError **error)
{
	...
	nlh = fu_devlink_cmd_prepare(&self->nlg, DEVLINK_CMD_GET, TRUE);

	/* Send enumeration request and receive responses */
	if (!fu_devlink_msg_send_recv(&self->nlg, nlh, fu_devlink_enumerate_cb, self, error)) {
