...
static int
fu_devlink_flash_mon_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	mnl_attr_parse(nlh, sizeof(*genl), fu_devlink_attr_cb, tb);
...
static gpointer
fu_devlink_flash_mon_thread(gpointer user_data)
{
	...
	if (!fu_devlink_gen_socket_open(&nlg, NULL))
		return NULL;
	if (!fu_devlink_mcast_group_subscribe(&nlg))
		return NULL;
	if (!fu_devlink_msg_recv_run(&nlg, 0, fu_devlink_flash_mon_cb, ctx, NULL))
		return NULL;
...
/* Send flash command */
static gboolean
fu_devlink_flash_send(FuDevlinkDevice *self,
		      ...
		      GError **error)
{
	...
	if (!fu_devlink_gen_socket_open(&nlg, error))
		return FALSE;

	/* Prepare flash update command */
	nlh = fu_devlink_cmd_prepare(&nlg, DEVLINK_CMD_FLASH_UPDATE, FALSE);
	...
	if (!fu_devlink_msg_send(&nlg, nlh, error))
		return FALSE;
...
static gboolean
fu_devlink_flash(FuDevlinkDevice *self,
		 const gchar *component_name,
		 const gchar *filename,
		 FuProgress *progress,
		 GError **error)
{
	...
	flash_mon_thread =
	    g_thread_new("devlink-flash-mon", fu_devlink_flash_mon_thread, &flash_mon_ctx);
	if (!flash_mon_thread) {
		g_set_error(error,
			    FWUPD_ERROR,
	...
	ret = fu_devlink_flash_send(self, component_name, filename, error);
					   ...
					   GError **error)
{
	...
	ret = fu_devlink_flash(self, component_name, fw_basename, progress, error);
...
fu_devlink_device_get_version_table(const struct nlmsghdr *nlh)
{
	...
	mnl_attr_for_each(attr, nlh, sizeof(struct genlmsghdr))
	{
		...
		if (mnl_attr_parse_nested(attr, fu_devlink_attr_cb, ver_tb) != MNL_CB_OK)
			continue;
...
fu_devlink_device_get_info(FuDevice *device, GError **error)
{
	...
	nlh = fu_devlink_cmd_prepare(&nlg, DEVLINK_CMD_INFO_GET, FALSE);
	mnl_attr_put_strz(nlh, DEVLINK_ATTR_BUS_NAME, self->bus_name);
	mnl_attr_put_strz(nlh, DEVLINK_ATTR_DEV_NAME, self->dev_name);
	...
	if (!fu_devlink_msg_send_recv(&nlg, nlh, fu_devlink_device_info_cb, device, error)) {
...
/* Context for component-specific version reload */
typedef struct {
	FuDevice *component_device;
	const gchar *component_name;
	gboolean found;
} FuDevlinkComponentReloadCtx;

/* Parse devlink info and update specific component version */
static int
fu_devlink_device_component_reload_cb(const struct nlmsghdr *nlh, void *data)
{
	struct genlmsghdr *genl = mnl_nlmsg_get_payload(nlh);
	struct nlattr *tb[DEVLINK_ATTR_MAX + 1] = {};
	FuDevlinkComponentReloadCtx *ctx = (FuDevlinkComponentReloadCtx *)data;
	g_autofree gchar *version_running = NULL;
	g_autofree gchar *version_stored = NULL;
	struct nlattr *attr;
	...
	/* Process running version information */
	mnl_attr_for_each(attr, nlh, sizeof(struct genlmsghdr))
	{
		struct nlattr *ver_tb[DEVLINK_ATTR_MAX + 1] = {};
		const gchar *name, *value;

		/* Parse nested attributes for version information */
		if (mnl_attr_parse_nested(attr, fu_devlink_attr_cb, ver_tb) != MNL_CB_OK)
			continue;
		...
		name = mnl_attr_get_str(ver_tb[DEVLINK_ATTR_INFO_VERSION_NAME]);
		if (g_strcmp0(name, ctx->component_name) != 0)
			continue;
		...
		case DEVLINK_ATTR_INFO_VERSION_RUNNING:
			if (version_running) {
				g_debug("component '%s' running version already set to '%s'",
					ctx->component_name,
					version_running);
				continue;
			}
			version_running = g_strdup(value);
			break;
		case DEVLINK_ATTR_INFO_VERSION_STORED:
			if (version_stored) {
				g_debug("component '%s' stored version already set to '%s'",
					ctx->component_name,
					version_stored);
				continue;
			}
			version_stored = g_strdup(value);
			break;
		default:
			continue;
		}
	...
	if (!version_running && !version_stored) {
		g_debug("component '%s' has no version information", ctx->component_name);
		return MNL_CB_OK;
	}

	ctx->found = TRUE;

	if (version_stored && g_strcmp0(version_running, version_stored)) {
		fu_device_set_version(ctx->component_device, version_stored);
	} else {
		fu_device_set_version(ctx->component_device, version_running);
		fu_device_add_flag(ctx->component_device, FWUPD_DEVICE_FLAG_NEEDS_ACTIVATION);
	}

	return MNL_CB_OK;
}

/* Reload version for a specific component */
gboolean
fu_devlink_device_reload_component_version(FuDevlinkDevice *self,
					   FuDevice *component_device,
					   const gchar *component_name,
					   GError **error)
{
	g_auto(FuDevlinkGenSocket) nlg = {};
	struct nlmsghdr *nlh;
	FuDevlinkComponentReloadCtx ctx = {.component_device = component_device,
	...
	g_debug("reloading version for component '%s' on %s/%s",
		component_name,
		self->bus_name,
		self->dev_name);

	/* Send command and process response */
	if (!fu_devlink_msg_send_recv(&nlg,
				      nlh,
				      fu_devlink_device_component_reload_cb,
				      &ctx,
				      error)) {
		g_prefix_error(error, "failed to get device info: ");
		return FALSE;
	...
	if (!ctx.found) {
		g_debug("component '%s' not found in devlink info", component_name);
		return FALSE;
	}
...
fu_devlink_device_activate(FuDevice *device, FuProgress *progress, GError **error)
{
	...
	nlh = fu_devlink_cmd_prepare(&nlg, DEVLINK_CMD_RELOAD, FALSE);
	mnl_attr_put_strz(nlh, DEVLINK_ATTR_BUS_NAME, self->bus_name);
	mnl_attr_put_strz(nlh, DEVLINK_ATTR_DEV_NAME, self->dev_name);
...
fu_devlink_device_new(FuContext *ctx, const gchar *bus_name, const gchar *dev_name)
{
	...
	fu_device_set_vendor(FU_DEVICE(self), "Linux Foundation");
	fu_device_set_vid(FU_DEVICE(self), 0x1D6B);
