...
struct nlmsghdr *
fu_devlink_msg_prepare(void *buf,
		       uint32_t nlmsg_type,
		       gboolean dump,
		       void *extra_header,
...
static int
fu_devlink_nlmsgerr_attr_cb(const struct nlattr *attr, void *data)
{
...
static gboolean
fu_devlink_error_cb_extack(const struct nlmsghdr *nlh, GError **error)
{
	...
	if (mnl_attr_parse(nlh, hlen, fu_devlink_nlmsgerr_attr_cb, tb) != MNL_CB_OK)
		return FALSE;
...
static int
fu_devlink_error_cb(const struct nlmsghdr *nlh, void *data)
{
	const struct nlmsgerr *err = mnl_nlmsg_get_payload(nlh);
	...
	if (!err->error)
		return MNL_CB_STOP;
	if (fu_devlink_error_cb_extack(nlh, ctx->error))
		return MNL_CB_ERROR;
...
static int
fu_devlink_noop_cb(const struct nlmsghdr *nlh, void *data)
{
	return MNL_CB_STOP;
}

static int
fu_devlink_done_cb(const struct nlmsghdr *nlh, void *data)
{
	return MNL_CB_STOP;
}
...
static int
fu_devlink_data_cb(const struct nlmsghdr *nlh, void *data)
{
	FuDevlinkCbCtx *ctx = data;
...
static const mnl_cb_t fu_devlink_cbs[NLMSG_MIN_TYPE] = {
    [NLMSG_NOOP] = fu_devlink_noop_cb,
    [NLMSG_ERROR] = fu_devlink_error_cb,
    [NLMSG_DONE] = fu_devlink_done_cb,
    [NLMSG_OVERRUN] = fu_devlink_noop_cb,
};

/* Receive and run callback on netlink messages with proper error handling */
gboolean
fu_devlink_msg_recv_run(FuDevlinkGenSocket *nlg,
			guint32 seq,
			mnl_cb_t cb,
			void *data,
			GError **error)
...
gboolean
fu_devlink_msg_send_recv(FuDevlinkGenSocket *nlg,
			 struct nlmsghdr *nlh,
			 mnl_cb_t cb,
			 void *data,
			 GError **error)
{
	...
	return fu_devlink_msg_recv_run(nlg, seq, cb, data, error);
}

gboolean
fu_devlink_msg_send(FuDevlinkGenSocket *nlg, struct nlmsghdr *nlh, GError **error)
{
	return fu_devlink_msg_send_recv(nlg, nlh, NULL, NULL, error);
}

/* Generic Netlink control attribute callback */
static int
genl_ctrl_attr_cb(const struct nlattr *attr, void *data)
{
	const struct nlattr **tb = data;
	int type = mnl_attr_get_type(attr);
...
static int
genl_mcast_group_attr_cb(const struct nlattr *attr, void *data)
{
	const struct nlattr **tb = data;
	int type = mnl_attr_get_type(attr);
...
static int
genl_family_get_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	mnl_attr_parse(nlh, sizeof(*genl), genl_ctrl_attr_cb, tb);
	...
	mnl_attr_for_each_nested(mcgrp, tb[CTRL_ATTR_MCAST_GROUPS])
	{
		...
		mnl_attr_parse_nested(mcgrp, genl_mcast_group_attr_cb, tb_grp);
...
static gboolean
genl_family_get(FuDevlinkGenSocket *nlg, const gchar *family_name, GError **error)
{
	...
	nlh = fu_devlink_msg_prepare(nlg->buf, GENL_ID_CTRL, FALSE, &hdr, sizeof(hdr));
	mnl_attr_put_strz(nlh, CTRL_ATTR_FAMILY_NAME, family_name);
	return fu_devlink_msg_send_recv(nlg, nlh, genl_family_get_cb, nlg, error);
}

/* Open generic netlink socket for devlink family */
gboolean
fu_devlink_gen_socket_open(FuDevlinkGenSocket *nlg, GError **error)
{
	gint one = 1;
	gint rc;
	...
	if (!genl_family_get(nlg, DEVLINK_GENL_NAME, error)) {
...
void
fu_devlink_gen_socket_close(FuDevlinkGenSocket *nlg)
{
	if (nlg->nl) {
		mnl_socket_close(nlg->nl);
		nlg->nl = NULL;
...
gint
fu_devlink_gen_socket_get_fd(FuDevlinkGenSocket *nlg)
{
...
struct nlmsghdr *
fu_devlink_cmd_prepare(FuDevlinkGenSocket *nlg, guint8 cmd, gboolean dump)
{
	struct genlmsghdr hdr = {
	    .cmd = cmd,
	...
	return fu_devlink_msg_prepare(nlg->buf, nlg->family_id, dump, &hdr, sizeof(hdr));
}

gboolean
fu_devlink_mcast_group_subscribe(FuDevlinkGenSocket *nlg)
{
	...
	if (rc < 0) {
		g_debug("failed to subscribe to devlink notifications: %s", g_strerror(errno));
		return FALSE;
	}
...
gboolean
fu_devlink_mcast_group_unsubscribe(FuDevlinkGenSocket *nlg)
{
	guint32 devlink_config_grp = nlg->config_group_id;
	gint rc;
	...
	rc = mnl_socket_setsockopt(nlg->nl,
				   ...
				   sizeof(devlink_config_grp));
	if (rc < 0) {
		g_debug("failed to unsubscribe from devlink notifications: %s", g_strerror(errno));
		return FALSE;
	}
...
int
fu_devlink_attr_cb(const struct nlattr *attr, void *data)
{
	const struct nlattr **tb = data;
	int type = mnl_attr_get_type(attr);
	int rc;
